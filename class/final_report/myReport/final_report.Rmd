...
## 0. 要旨
本レポートでは2000年度以降の各市町村における観光入込客数を地図で描画し、俯瞰して見ることで北海道の各地域、年度における観光業について考察した。
...
そこで、北海道の観光入込客数を各市町村、各年度ごとに地図で描画し、俯瞰的に考察しようと考えた。
...
PDF形式のデータは処理が難しいため使用を断念し、2000年度以降のデータのみを使用した。
...
## 4. 結果
図1. 各年度における観光入込客数の合計(単位:人)
```{r sum_gif}
knitr::include_graphics("figures/anim_map_sum.gif")
```

図2. 各年度における観光入込客数の対前年度比(単位:%)
```{r rat_gif}
...
また、図2では2020年度になった途端にほとんどの市町村が対前年度比の観光入込客数が減少していることが分かった。
...
図2からは2020年度の対前年度比観光入込客数がほぼ全市町村で小さな値を取っていることが分かった。
